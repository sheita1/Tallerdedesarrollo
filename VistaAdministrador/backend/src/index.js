...
// ✅ Cargar variables de entorno desde Docker
dotenv.config();

import { fileURLToPath } from "url";
import { dirname, join } from "path";
import express, { json, urlencoded } from "express"; // Simplifiqué imports no usados como fs/mime/path repetidos
...
async function setupServer() {
  try {
    ...
    // -----------------------------------------------------------------
    // ✅ RUTAS DE API (AQUÍ ESTÁ LA CORRECCIÓN)
    // -----------------------------------------------------------------

    // ⚠️ CAMBIO CRÍTICO: Primero cargamos la ruta específica de patrimonios
    // Si ponemos primero "/api", el servidor bloquea el acceso a patrimonios.
    app.use("/api/patrimonios", patrimonioRoutes);

    // Luego cargamos el resto
    ...
    // Agregamos try/catch aquí para que no se caiga si ya existen los datos
    try {
        await createUsers();
        await createPatrimonios();
    } catch (e) {
        console.log("Datos iniciales ya existen o error menor:", e.message);
    }
